...
BOOTSTRAP_SCRIPT="$SCRIPT_DIR/bootstrap-runtime.sh"
...
build_chroot()
{
	...
	chmod +x "/tmp/${TMPNAME}"
	# JE: Need sudo
	sudo schroot --chroot ${CHROOT_NAME} -d /tmp --user root -- "/tmp/${TMPNAME}" --chroot "$@"
	rm -f "/tmp/${TMPNAME}"
	cp -f "$SCRIPT_DIR/write-manifest" "/tmp/${TMPNAME}"
	chmod +x "/tmp/${TMPNAME}"
	# JE: Need sudo
	sudo schroot --chroot ${CHROOT_NAME} -d /tmp --user root -- "/tmp/${TMPNAME}" /
	rm -f "/tmp/${TMPNAME}"
